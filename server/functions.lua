...
function GetPlayerDetails(src, config, channel)
    ...
    if GetConvar("steam_webApiKey", "false") ~= 'false' then
        ...
        _steamID = ""
        _steamURL = ""
        TriggerEvent('AppelBoy::errorLog', 'You need to set a steam api key in your server.cfg for the steam identifiers to work!.')
    end
    ...
    if config.Session or config.PlayTime and not webhooksFile[channel].Hide['PlayTime'] then
        if GetResourceState('AppelBoy_playTime') == "started" then
            playtime = exports.AppelBoy_playTime:getPlayTime(src)
            ...
            TriggerEvent('AppelBoy::errorLog', 'AppelBoy_playTime is not installed.')
            _total = ""
            _session = ""
        end
    else
    ...
    if args['channel'] == 'system' then
        message = {
            ...
            embeds = {{
                ...
                ["author"] = {
                    ["name"] = "AppelBoy",
                    ["icon_url"] = "https://redlinerp.es/i/DiscordIcon.png",
                    ["url"] = "https://discord.gg/AppelBoy"
                },
                ["title"] = "📢 SYSTEM",
                ["description"] = args['description'],
                ["footer"] = {
                    ["text"] = "2020 - "..os.date("%Y").." © AppelBoy • "..os.date("%x %X %p"),
                    ["icon_url"] = "https://redlinerp.es/i/DiscordIcon.png",
                },
            }}, 
            avatar_url = "https://redlinerp.es/i/DiscordIcon.png"
        }
    ...
    if webhooksFile[args.channel] then
        ...
        if args.player_id then
            if webhooksFile[args.channel].logHistory then
                TriggerClientEvent('AppelBoy:ClientLogStorage', args.player_id, { Channel = args.channel, Message = args.EmbedMessage, TimeStamp = os.date("%x %X %p") })
            end
        ...
        if args.player_2_id then
            if webhooksFile[args.channel].logHistory then
                TriggerClientEvent('AppelBoy:ClientLogStorage', args.player_2_id, { Channel = args.channel, Message = args.EmbedMessage, TimeStamp = os.date("%x %X %p") })
            end
        ...
        print(": Webhooks channel not found. ("..args.channel..")")        
    end
end

function getStatus()
	if status == 404 or status == 401 and webhooksFile[channel].webhook ~= "DISCORD_WEBHOOK" and webhooksFile[channel].webhook ~= "" then 
		print('^3Warn:  webhook. Possible invalid webhook for "'..channel..'" webhook. Status code: '..status)
	end
end
