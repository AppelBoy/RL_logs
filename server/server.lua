...
local rl_Debug = false -- Enable when you have issues or when asked by AppelBoy Staff
...
RegisterNetEvent('AppelBoy::Debug')
AddEventHandler('AppelBoy::Debug', log)

RegisterNetEvent('AppelBoy::errorLog')
AddEventHandler('AppelBoy::errorLog', errorLog)
...
RegisterNetEvent("AppelBoy:discordLogs")
AddEventHandler("AppelBoy:discordLogs", function(message, color, channel)
    discordLog(message, color, channel)
end)

RegisterNetEvent("AppelBoy:ClientUploadScreenshot")
AddEventHandler("AppelBoy:ClientUploadScreenshot", function(args)
...
exports('createLog', function(args)
	if args.screenshot then
		if GetResourceState('screenshot-basic') == "started" then
			...
			TriggerClientEvent('AppelBoy:ClientCreateScreenshot', args.player_id, args)
		else
...
AddEventHandler("ACCheatAlert", function(reason)
	...
	PerformHttpRequest('https://cdn.AppelBoy.dev/api/cheatAlert.php', function(err, text, headers)
	end, 'POST', json.encode(args), {
		['Content-Type'] = 'application/json' 
	})
	DropPlayer(source, '\nYou have been kicked by the AppelBoy Auto kick system.')
end)

AddEventHandler("playerConnecting", function(name, setReason, deferrals)
	local ids = ExtractIdentifiers(source)
	if cfgFile['AppelBoyGlobalBans'] then
		deferrals.defer()
		Wait(50)
		...
		if not bypass then			
			local args = { ['ids'] = ids }
			PerformHttpRequest('https://cdn.AppelBoy.dev/api/checkBan.php', function(err, text, headers)
				if text == nil then
...
RegisterServerEvent('AppelBoy:playerDied')
AddEventHandler('AppelBoy:playerDied',function(args)
...
RegisterServerEvent('AppelBoy:playerShotWeapon')
AddEventHandler('AppelBoy:playerShotWeapon', function(weapon)
...
RegisterServerEvent('AppelBoy:ClientDiscord')
AddEventHandler('AppelBoy:ClientDiscord', function(args)
	if args.screenshot then
		if GetResourceState('screenshot-basic') == "started" then
			local webhooksLaodFile = LoadResourceFile(GetCurrentResourceName(), "./config/webhooks.json")
			local webhooksFile = json.decode(webhooksLaodFile)
...
local storage = nil
RegisterNetEvent('AppelBoy:sendClientLogStorage')
AddEventHandler('AppelBoy:sendClientLogStorage', function(_storage)
	storage = _storage
end)
...
Commands.LogHistoryCommand = function(source, args, RawCommand)
	if GetResourceState('AppelBoy_Notify') == "started" then
		if IsPlayerAceAllowed(source, cfgFile.logHistoryPerms) then
			if tonumber(args[1]) then
				TriggerClientEvent('AppelBoy:getClientLogStorage', args[1])
				Wait(500)
				if tablelength(storage) == 0 then
					exports.AppelBoy_Notify:Notify({
		...
		errorLog('AppelBoy_Notify is not installed.')
	end
end
		...
		if source == 0 then
			if args[1] and has_val(GetPlayers(), args[1]) then
				if GetResourceState('screenshot-basic') == "started" then
					...
					TriggerClientEvent('AppelBoy:ClientCreateScreenshot', args[1], args)
...
RegisterServerEvent('AppelBoy:eventLoggerClient')
AddEventHandler('AppelBoy:eventLoggerClient', function(args)
	ServerFunc.CreateLog(args)
end)

if GetCurrentResourceName() ~= "" then
    errorLog('This recource should be named "" for the exports to work properly.')
end

RegisterNetEvent('AppelBoy:DropPlayer')
AddEventHandler('AppelBoy:DropPlayer', function(reason)
	local configFile = LoadResourceFile(GetCurrentResourceName(), "./config/config.json")
	local cfgFile = json.decode(configFile)
...
RegisterNetEvent('AppelBoy:getACConfig')
AddEventHandler('AppelBoy:getACConfig', function()
	local configFile = LoadResourceFile(GetCurrentResourceName(), "./config/ac_config.json")
	local cfgFile = json.decode(configFile)
	TriggerClientEvent('AppelBoy:SendACConfig', source, cfgFile)
end)
...
RegisterNetEvent("AppelBoy:resourceCheck")
AddEventHandler("AppelBoy:resourceCheck", function(rcList)
	local source = source
	Wait(50)
...
CreateThread( function()
	...
	SetConvarServerInfo("", "V"..version)	
	if version then
		PerformHttpRequest(
			'https://raw.githubusercontent.com/AppelBoy//master/json/version.json',
...
-------------------------------------------------------^0]]):format(
						...
						if cfgFile['DiscordUpdateNotify'] then
							ServerFunc.CreateLog({ description = "** Update V"..rv.version.."**\nDownload the latest update of  here:\nhttps://github.com/AppelBoy/RL_logs/releases/latest\n\n**Changelog:**\n"..rv.changelog, ping = true, channel = 'system'})
						end
					end
				else
					errorLog(' unable to check version')
				end
			end,
			'GET'
		)
