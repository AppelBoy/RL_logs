	...
	while true do
		...
		if IsEntityDead(GetPlayerPed(PlayerId())) then
			...
			if DeathReason == lang['DeathReasons'].Suicide or DeathReason == lang['DeathReasons'].Died then
				TriggerServerEvent('AppelBoy:playerDied', {
					type = 1, 
...
RegisterNetEvent('AppelBoy:ClientCreateScreenshot')
AddEventHandler('AppelBoy:ClientCreateScreenshot', function(args)
	if args.url ~= "" and args.url ~= nil and args.url ~= 'DISCORD_WEBHOOK' then
		exports['screenshot-basic']:requestScreenshotUpload(args.url, 'files[]', function(data)
			...
			TriggerServerEvent('AppelBoy:ClientUploadScreenshot', args)
		end)
	end
end)
		...
		if IsPedShooting(playerped) then
			if ClientWeapons.WeaponNames[tostring(GetSelectedPedWeapon(playerped))] then
				...
				if isLoggedWeapon then
					TriggerServerEvent('AppelBoy:playerShotWeapon', ClientWeapons.WeaponNames[tostring(GetSelectedPedWeapon(playerped))])
				end				
			else
				TriggerServerEvent('AppelBoy:playerShotWeapon', lang['WeaponFired'].Undefined)
				TriggerServerEvent('AppelBoy::Debug', 'Weapon not defined.', "Weapon not listed: "..tostring(GetSelectedPedWeapon(playerped)))
			end
		end
	end
end)
...
exports('discord', function(message, id, id2, color, channel)
	...
	TriggerServerEvent('AppelBoy:ClientDiscord', args)
	local resource = GetInvokingResource()
	TriggerServerEvent('AppelBoy::Debug', 'Server Old Export from '..resource)
end)

exports('createLog', function(args)
	TriggerServerEvent('AppelBoy:ClientDiscord', args)	
	local resource = GetInvokingResource()
	TriggerServerEvent('AppelBoy::Debug', 'Server New Export from '..resource)
end)

local clientStorage = {}
RegisterNetEvent('AppelBoy:ClientLogStorage')
AddEventHandler('AppelBoy:ClientLogStorage', function(args)
...
RegisterNetEvent('AppelBoy:getClientLogStorage')
AddEventHandler('AppelBoy:getClientLogStorage', function()
    TriggerServerEvent('AppelBoy:sendClientLogStorage', clientStorage)
end)

CreateThread(function()
	TriggerServerEvent('AppelBoy:getACConfig')
	while true do
		Wait(60 * 1000)
		TriggerServerEvent('AppelBoy:getACConfig')
	end
...
if cfgFile['EnableAcFunctions'] then
	local acConfig = {}
	RegisterNetEvent('AppelBoy:SendACConfig')
	AddEventHandler('AppelBoy:SendACConfig', function(_config)
		acConfig = _config
	end)
				...
				for k, v in pairs(acConfig['BlacklistedVehicles']) do
					if (IsVehicleModel(vehicle, v)) then
						...
						if GetPlayerServerId(PlayerId()) ~= nil then
							TriggerServerEvent('AppelBoy:ClientDiscord', {EmbedMessage = lang['AntiCheat'].ACBlacklistedVehLog:format(GetPlayerName(PlayerId()), v), player_id = GetPlayerServerId(PlayerId()), channel = 'AntiCheat'})
						end
						...
						if acConfig['KickSettings'].BlacklistedVehicles then
							if warnLimit == acConfig['KickSettings'].BlacklistedVehicleLimit then
								TriggerServerEvent('AppelBoy:DropPlayer', lang['AntiCheat'].ACBlacklistedVehKick)
							end
						end
					end
				end
			...
			while not finished do
				...
				for k,v in pairs(acConfig['BlacklistedObjects']) do
					if (GetEntityModel(object) == GetHashKey(v)) then
						DeleteObject(object)
						if GetPlayerServerId(PlayerId()) ~= nil then
							TriggerServerEvent('AppelBoy:ClientDiscord', {EmbedMessage = lang['AntiCheat'].BlacklistedObject:format(v), player_id = GetPlayerServerId(PlayerId()), channel = 'AntiCheat'})
						end
					end
				end
		...
		while true do
			...
			for k,v in pairs(acConfig['BlacklistedKeys']) do
				if IsControlJustReleased(0, tonumber(k)) and not IsNuiFocused() then
					Wait(500)
					if GetPlayerServerId(PlayerId()) ~= nil then
						TriggerServerEvent('AppelBoy:ClientDiscord', {EmbedMessage = lang['AntiCheat'].BlacklistedKey:format(k, v), player_id = GetPlayerServerId(PlayerId()), screenshot = true, channel = 'AntiCheat'})
					end
				end
			end
		end
	...
	CreateThread(function()
		while true do
			...
			Wait(5000)
			TriggerServerEvent("AppelBoy:resourceCheck", resourceList)      
		end
	end)
				...
				for k,x in pairs(acConfig['BlacklistedCommands']) do
					for k,z in pairs(prefixCheck) do
						if string.lower(v.name) == string.lower(z..""..x) then
							if GetPlayerServerId(PlayerId()) ~= nil then
								TriggerServerEvent('AppelBoy:ClientDiscord', {EmbedMessage = lang['AntiCheat'].BlacklistedCommand:format(x), player_id = GetPlayerServerId(PlayerId()), channel = 'AntiCheat'})
							end
							TriggerServerEvent('ACCheatAlert', 'Blacklisted Command: '..z..''..x)
							if acConfig['KickSettings'].BlacklistedCommands then
								TriggerServerEvent('AppelBoy:DropPlayer', lang['AntiCheat'].BlacklistedCommandKick)
							end
						end
					end
				end
...
if type(eventsFile) == "table" then
	for k,v in pairs(eventsFile) do
		if not v.Server then
			TriggerServerEvent('AppelBoy::Debug', 'Added Client Event Log: '..v.Event)
			AddEventHandler(v.Event, function()
				TriggerServerEvent('AppelBoy:ClientDiscord', {EmbedMessage = 'EventLogger: '..v.Message, channel = v.Channel})
				TriggerServerEvent('AppelBoy:eventLoggerClient', {EmbedMessage = 'EventLogger: '..v.Message, channel = v.Channel})
			end)
		end
	end
end
